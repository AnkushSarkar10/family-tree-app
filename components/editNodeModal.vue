...
const genderRef = ref(props.data.gender)
const spouceGenderrRef = ref(props.data.spouseGender)
const gendeOptions = ref([
    { text: 'Male', value: 'M' },
    { text: 'Female', value: 'F' }
])
const nodeName = ref(props.data.name);
const nodeSpouseName = ref(props.data.spouseName);
const client = useSupabaseClient<Database>();

const editNode = async () => {
    const { error } = await client
        .from('family_members')
        .update({ name: nodeName.value, gender: genderRef.value, spouseName: nodeSpouseName.value, spouseGender: spouceGenderrRef.value })
            ...
            <div class="px-4 py-5">
                ...
                <div class="mt-6">
                    <div class="w-full space-y-6">
                        <div class="w-full">
                            <div class=" relative ">
                                <input v-model="nodeName" type="text" id="search-form-price"
                        ...
                        <div v-if="data.hasSpouse == 't'" class="w-full">
                            <div class=" relative ">
                                <input v-model="nodeSpouseName" type="text" id="search-form-price"
                                    class=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-transparent"
                                    placeholder="Spouce Name" />
                            </div>
                        </div>
                        <select v-if="data.hasSpouse == 't'" v-model="spouceGenderrRef"
                            class="w-full block px-3 py-2 bg-white border border-gray-300 appearance-none rounded-md shadow-sm focus:outline-none focus:ring-teal-600 focus:border-teal-500">
                            <option v-for="option in gendeOptions" :key="option.value" :value="option.value">
                                {{ option.text }}
                            </option>
                        ...
                        <div class="w-full relative inline-block focus:outline-none focus:ring">
                            ...
                            <button @click="editNode"
                                ...
                                <Icon name="material-symbols:edit-outline-rounded" color="black" class="text-lg mr-1" />
                                Edit node
                            </button>
                        </div>
                    </div>
